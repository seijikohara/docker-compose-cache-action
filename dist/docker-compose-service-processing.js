...
import { generateCacheKey, generateCacheKeyPrefix, generateManifestCacheKey, generateManifestPath, generateTarPath, readManifestFromFile, restoreFromCache, saveManifestToCache, saveToCache, } from './cache.js';
import { inspectImageLocal, inspectImageRemote, loadImageFromTar, pullImage, saveImageToTar, } from './docker-command.js';
/**
 ...
 */
async function tryRestoreFromCacheWithoutDigest(completeImageName, imageNameWithoutTag, imageTag, platform, cacheKeyPrefix) {
    ...
    const cacheResult = await restoreFromCache([fallbackTarPath], `${cacheKeyPrefixWithoutDigest}-fallback`, // This won't match exactly
 ...
 */
export async function processService(serviceDefinition, cacheKeyPrefix, skipLatestCheck, forceRefresh = false) {
    ...
    if (!manifest?.digest) {
        ...
        if (skipLatestCheck && !forceRefresh) {
            const fallbackResult = await tryRestoreFromCacheWithoutDigest(completeImageName, imageNameWithoutTag, imageTagOrLatest, serviceDefinition.platform, cacheKeyPrefix);
            if (fallbackResult) {
                core.warning(`Registry unavailable for ${completeImageName}. Using cached version. ` +
    ...
    const imageCacheKey = generateCacheKey(cacheKeyPrefix, imageNameWithoutTag, imageTagOrLatest, serviceDefinition.platform, imageDigest);
    const imageTarPath = generateTarPath(imageNameWithoutTag, imageTagOrLatest, serviceDefinition.platform, imageDigest);
    const manifestCacheKey = generateManifestCacheKey(cacheKeyPrefix, imageNameWithoutTag, imageTagOrLatest, serviceDefinition.platform, imageDigest);
    const manifestPath = generateManifestPath(imageNameWithoutTag, imageTagOrLatest, serviceDefinition.platform, imageDigest);
...
//# sourceMappingURL=docker-compose-service-processing.js.map