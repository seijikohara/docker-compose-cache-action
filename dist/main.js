...
import { buildProcessedImageList, calculateActionSummary, createActionSummary, logActionCompletion, setActionOutputs, } from './action-outputs.js';
 ...
 */
function getSkipDigestVerification() {
    ...
    if (skipLatestCheckInput !== '') {
        core.warning("The 'skip-latest-check' input is deprecated and will be removed in a future major version. " +
...
export async function run() {
    ...
    try {
        ...
        const serviceProcessingResults = await Promise.all(targetServices.map(async (currentService) => {
            ...
            const serviceResult = await processService(currentService, actionConfig.cacheKeyPrefix, actionConfig.skipDigestVerification, actionConfig.forceRefresh);
